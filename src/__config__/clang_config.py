...
clang_libdir = (
    subprocess.check_output(["llvm-config", "--libdir"]).decode().rstrip("\r\n")
)

libclang, *other = [
    lib
    for lib in os.listdir(clang_libdir)
    if lib.startswith("libclang.") and (lib.endswith("so") or lib.endswith("dll"))
]
