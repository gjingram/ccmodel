...
class ParseObject(object):
    ...
    def __init__(self, node: typing.Union[cindex.Cursor, None], force: bool = False):
        ...
        self.info["canonical_type"] = node.canonical.type.spelling if node is not None else ''
    ...
    def extend_children(self, node: 'cindex.Cursor', ext_array: typing.List['cindex.Cursor']) -> None:
    ...
    def determine_scope_name(self, node: cindex.Cursor) -> None:
        ...
        if node is not None and node.kind == cindex.CursorKind.NAMESPACE \
            ...
            self["is_anonymous"] = True
                ...
                (node.kind == cindex.CursorKind.CLASS_DECL or \
                node.kind == cindex.CursorKind.STRUCT_DECL or \
        ...
        if node is not None:
            ...
            if "export_to_scope" not in dir(parent):
                if parent.spelling == "" and \
                        ...
                        parent.kind == cindex.CursorKind.UNION_DECL):
                    parent.export_to_scope = True
                elif parent.kind == cindex.CursorKind.ENUM_DECL and \
                        not parent.is_scoped_enum():
                    parent.export_to_scope = True
                else:
            ...
            while parent.kind is not cindex.CursorKind.TRANSLATION_UNIT:
                ...
                if parent.kind is cindex.CursorKind.NAMESPACE and parent.spelling == "":
                    ...
                    if parent.semantic_parent.kind is not cindex.CursorKind.TRANSLATION_UNIT:
                        name_use = parent.semantic_parent.spelling
                        namedisp_use = parent.semantic_parent.displayname
                elif node.semantic_parent.kind is cindex.CursorKind.ENUM_DECL and \
                        node.semantic_parent.export_to_scope:
                    if parent.semantic_parent.kind is not cindex.CursorKind.TRANSLATION_UNIT:
                        name_use = parent.semantic_parent.spelling
                        namedisp_use = parent.semantic_parent.displayname
                else:
                ...
                if not parent.export_to_scope or \
                        parent.semantic_parent.kind == cindex.CursorKind.TRANSLATION_UNIT:
        ...
        self["scoped_displayname"] = "::".join([self["scopedisp_name"], self["displayname"]]) \
                if self["scope_name"] != "" \
    ...
    def children(self, nodes: typing.List[cindex.Cursor],
            kind: cindex.CursorKind) -> typing.List[cindex.Cursor]:
    ...
    def do_handle(self, node: cindex.Cursor) -> bool:
        ...
        parse_also = self["scoped_id"] in self["header"].parser.parse_also
        if parse_also:
            p_also = self["header"].parser.parse_also[self["scoped_id"]]
            self.force_search = p_also["force_search"]
            self["force_parse"] = p_also["force_parse"]
        allowed = self.get_name() != "" or \
                (self.get_name() == "" and self["export_to_scope"]) or \
                (self.get_name() == "" and node.kind == cindex.CursorKind.NAMESPACE) or \
                parse_also
        self.handle_object = self["force_parse"] or (in_unit and \
                self["id"] not in self["header"].parser.get_excludes() and \
                allowed)
    ...
    def object_in_unit(self, node: cindex.Cursor) -> bool:
        return node.displayname == self["header"].header_file or \
                node.location.file.name in self["header"].parser.headers

    def create_clang_child_object(self, node: cindex.Cursor) -> 'ParseObject':
        cpo_class = self.get_child_type(node)
        tparents = self["template_parents"] if not self["is_template"] else \
